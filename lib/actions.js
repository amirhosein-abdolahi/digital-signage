...
import { addNew, deleteNew, editNew } from "./news";
import { addSubtitle, deleteSubtitle } from "./subtitles";
...
export async function editANew(slug, file, formData) {
  const _new = {
    title: formData.get("title"),
    image: formData.get("image"),
  };

  await editNew(slug, file, _new);
}
...
export async function deleteASubtitle(id) {
